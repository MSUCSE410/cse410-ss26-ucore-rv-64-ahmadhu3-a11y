...
void iupdate(struct inode *ip)
{
	...
	dip->type = ip->type;
	dip->size = ip->size;
	dip->nlink = ip->nlink; // LAB4: you may need to update link count here
...
void ivalid(struct inode *ip)
{
	...
	if (ip->valid == 0) {
		...
		ip->type = dip->type;
		ip->size = dip->size;
		ip->nlink = dip->nlink; // LAB4: You may need to get lint count here
...
int dirunlink(struct inode *dp, char *name)
{
	...
	// dirlookup gives us the inode AND sets off to where the entry sits in dp
	if ((ip = dirlookup(dp, name, &off)) == 0)
		return -1;
 
	// drop the ref dirlookup gave us — we only needed off
	iput(ip);
 
	// zero out the dirent to mark the slot as empty
